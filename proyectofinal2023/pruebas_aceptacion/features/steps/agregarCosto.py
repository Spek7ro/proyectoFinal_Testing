...
def step_impl(context):
    context.driver.find_element(By.XPATH, '//*[@id="collapseUtilities3"]/div/a[2]').click()
...
@then(u'puedo ver la descripcion del costo "{descripcion}" en la lista de costos')
def step_impl(context, descripcion):
    ...
    for costo in costos:
        if costo.find_element(By.XPATH, 'td[1]').text == descripcion:
            assert True, f"El descripcion {descripcion} no se encuentra en la lista de costos"
    time.sleep(2)

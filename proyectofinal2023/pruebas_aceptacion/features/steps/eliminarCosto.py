...
def step_impl(context):
    context.driver.find_element(By.XPATH, '//*[@id="dataTable"]/tbody/tr[4]/td[6]/a[1]').click()
...
def step_impl4(context, descripcion):
    costos = context.driver.find_elements(By.XPATH, '//*[@id="content"]/div/div/div/div[2]/div/div[1]/table/tbody/tr')
    for costo in costos:
        if costo.find_element(By.XPATH, 'td[1]').text == descripcion:
            assert True, f"La descripcion {descripcion} se encuentra en la lista de costos"
    time.sleep(2)

...
def step_impl(context):
    context.driver.find_element(By.XPATH, '//*[@id="collapseUtilities2"]/div/a[2]').click()
...
def step_impl(context, limite):
    context.driver.find_element(By.NAME, 'limite_presupuestario').send_keys(limite)
...
def step_impl(context):
    context.driver.find_element(By.XPATH, '//*[@id="content"]/div/div/div/form/button').click()
        
@then(u'puedo ver el id de la cuenta "{id_cuenta}" en la lista de cuentas bancarias')
def step_impl(context, id_cuenta):
    ...
    for cuenta in cuentas:
        if cuenta.find_element(By.XPATH, 'td[1]').text == id:
            assert True, f"El id {id_cuenta} no se encuentra en la lista de cuentas bancarias"
    time.sleep(2)

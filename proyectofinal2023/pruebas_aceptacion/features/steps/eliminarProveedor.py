...
def step_impl2(context):
    context.driver.find_element(By.XPATH, '//*[@id="dataTable"]/tbody/tr[3]/td[9]/a[1]').click()


@when(u'presiono el boton de confirmar eliminación')
def step_impl3(context):
    context.driver.find_element(By.XPATH, '//*[@id="content"]/div/div/form/button').click()
    time.sleep(3)
...
def step_impl4(context, rfc):
    ...
    for proveedor in proveedores:
        if proveedor.find_element(By.XPATH, 'td[1]').text == rfc:
            assert False, f"El proveedor con rfc {rfc} se encuentra en la lista de proveedores"
    time.sleep(3)
